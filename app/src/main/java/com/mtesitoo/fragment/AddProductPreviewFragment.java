...
import com.mtesitoo.helper.IntegerHelper;
...
public class AddProductPreviewFragment extends Fragment {
    ...
    private void populateScreen() {
        ...
        if (!quantityData.get(Constants.PRODUCT_CATEGORY_KEY).isEmpty()) {
            ...
            //Added Integerhelper class to wrap parsing into try catch block to avoid crash if no quantity is entered
            Integer unitId = IntegerHelper.parseInt(quantityData.get(Constants.PRODUCT_UNITS_KEY));
            for (Unit u : units) {
                if (u.getId() == unitId) {
